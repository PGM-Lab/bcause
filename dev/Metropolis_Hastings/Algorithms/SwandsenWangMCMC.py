    ...
    def _updated_factor(self, U) -> MultinomialFactor:
        ...
        if self._is_prior == True:
            ...
            for k in unique_keys:
                ...
                # ---> FIXED: Restored dynamic clustering (Removed the hardcoded (0, 0, 0)) <---
                idx_k = [i for i, val in enumerate(keys) if val == k]
                u_k = u_old[idx_k]
                S = sampling_set[k]
                ...
                for v in unique_u:
                    ...
                    # Because we move N_c rows at once, the probability ratio is raised to the power of N_c
                    # We use logs to prevent massive math overflows (e.g., 0.5 ^ 100 = 0.0)
                    log_ratio = N_c * (np.log(theta_current[v_prop] + 1e-10) - np.log(theta_current[v] + 1e-10))

                    # ---> ADDED: Track that N_c proposals were made <---
                    n_proposal += N_c

                    # 5. Accept or Reject the WHOLE cluster
                    if np.log(np.random.rand()) < min(log_ratio, 0.0):
                        u_new[cluster_idx] = v_prop
                        # ---> ADDED: Track that N_c proposals were accepted <---
                        n_accepted += N_c

            # Update the global samples
            self._int_exo_samples[U] = u_new

            # ---> ADDED: Apply the tracked values to the global history <---
