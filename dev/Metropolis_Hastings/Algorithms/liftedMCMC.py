...
class MetropolisHastingsSampling(IterativeParameterLearning):
    ...
    def initialize(self, data: pd.DataFrame, **kwargs):
        ...
        for U in self._trainable_vars:
            ...
            # --- ADD THIS BLOCK ---
            # Precompute lifted MCMC transitions to eliminate O(N) lookups
            self._mcmc_transitions[U] = {}
            for k, states in self._int_sampling_set[U].items():
                n = len(states)
                self._mcmc_transitions[U][k] = {
                    v: {
                        1: states[(i + 1) % n],
                        -1: states[(i - 1) % n]
            ...
            # Only if method is direction
            self._directions[U] = np.random.choice([-1, 1], size=len(self._int_exo_samples[U]))
    ...
    def _hash_map_sampling(self,U):
        ...
        int_sampling_series = sampling_set.map(lambda s: tuple(sorted(val_map[v] for v in s)))
    ...
    def lifted_MCMC(self, set, data, direction):
        actual_index = list(set).index(data)
        target_index = (actual_index + direction) % len(set)
    ...
    def _updated_factor(self, U) -> MultinomialFactor:
        ...
        if self._is_prior == True:
            ...
            # u_new = [ self.lifted_MCMC(sampling_set[k], v, self._directions[U][i]) for i, (k, v) in enumerate(zip(data["key"].values, u_old))]
            # Local references inside loops are slightly faster in Python
            transitions = self._mcmc_transitions[U]
            keys = data["key"].values
            dirs = self._directions[U]

            # O(1) dictionary lookup completely bypasses the lifted_MCMC function
            u_new = [transitions[k][v][d] for k, v, d in zip(keys, u_old, dirs)]
            ...
            self._directions[U] = np.where(flag == 1, self._directions[U], -self._directions[U])
...
if __name__ == "__main__":
    ...
    # path_model = "/Users/antoniogonzalezalves/Documents/s23/simple_nparents2_nzr02_zdr05_2.uai"
    # path_data = "/Users/antoniogonzalezalves/Documents/s23/simple_nparents2_nzr02_zdr05_2.csv"

    # m = StructuralCausalModel.read(path_model)
    ...
    mhs.run(data[m.endogenous], max_iter=1000)
