...
def test_model():
    ...
    assert ((p1*f1)**"U1").values == [0.5446886440624504, 0.45531135593754957]
...
def test_qfact_model():
    ...
    assert qfact["V1"].values == [0.5446886440624504, 0.45531135593754957]
    assert qfact["V2"].values == [0.17548216319464768,0.8245178368053524,0.6592091722110839,0.34079082778891606]
    assert qfact["V3"].values  == [0.3769378426382723,0.6230621573617277,0.956425026944555,0.04357497305544491]
    assert qfact["V4"].values[3:7] == [0.0, 0.5628335477556848, 0.4371664522443152, 0.7995086859069446]

def test_qfact_data():
    qfact = model.get_qfactorisation(data)
    assert qfact["V1"].values == [0.53, 0.47]
    assert qfact["V2"].values == [0.1320754716981132,0.8679245283018868,0.6425531914893617,0.3574468085106383]
    assert qfact["V3"].values  == [0.33870967741935487,0.6612903225806451,0.9426751592356687,0.057324840764331204]
    assert qfact["V4"].values[3:7] == [0.0, 0.5227272727272727, 0.47727272727272724, 0.8]
